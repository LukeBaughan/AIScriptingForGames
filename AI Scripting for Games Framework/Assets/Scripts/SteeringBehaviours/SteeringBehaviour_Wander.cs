...
public class SteeringBehaviour_Wander : SteeringBehaviour
{
    ...
    public override Vector2 CalculateForce()
    {
        ...
        m_wanderPosition += Maths.Normalise(m_Manager.m_Entity.m_Velocity) * m_WanderOffset; 
        ...
        Vector2 seekPositionVector = m_wanderPosition - new Vector2(transform.position.x, transform.position.y);
    ...
    //// Gets a random point on a circle (Min and max values are -1 and 1)
    //float radians = Mathf.Acos(Random.Range(-1f, 1f));
    //radians = radians + (Random.value< 0.5f ? 0 : Mathf.PI);
    //    m_PointOnCircle = m_WanderRadius* Mathf.Sin(radians) * Mathf.Cos(radians)* Vector2.one;

    //// Sets the circle's position to be in front of the entity
    //m_CirclePosition = (new Vector2(transform.position.x, transform.position.y) + Maths.Normalise(m_Manager.m_Entity.m_Velocity)) * m_WanderOffset;

    //    m_TargetPosition = m_PointOnCircle + m_CirclePosition;
